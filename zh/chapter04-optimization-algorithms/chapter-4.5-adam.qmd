...
title: "Chapter 4.5 Adam：Momentum 与 RMSprop 的结合"
order: 5
---

前面几节讨论的基础 SGD 有两个直接问题。

第一，mini-batch SGD 的梯度带有采样噪声。每一步只使用当前 batch 的梯度，参数更新方向容易抖动，训练初期尤为明显。

Momentum 不直接依赖当前梯度，而是维护历史梯度方向的滑动平均：
...
第二，不同参数的梯度尺度可能相差很大。所有参数共享同一个学习率时，一部分参数可能更新过快，另一部分则更新过慢。

RMSprop 为每个参数维护平方梯度的滑动平均：
...
目标很明确：更新方向不应由当前 batch 的噪声完全控制，同时每个参数应有自适应的有效学习率。
...
先比较 SGD、Momentum 与 RMSprop 的更新规则，再确定 Adam 的位置。
...
该递推式与 momentum 相同：当前梯度 $g_t$ 与过去的一阶矩估计共同决定新状态。较大的 $\beta_1$ 增加历史方向的权重，使更新更平滑；较小的 $\beta_1$ 则提高当前梯度的影响，使更新更快响应新信息。
...
该值通常比 $\beta_1$ 更接近 1，因此二阶矩估计具有更长的记忆。梯度方向需要较快响应当前 batch，而梯度尺度估计通常需要更稳定。
...
但这两个递推式还缺少一个步骤：**偏差修正（bias correction）**。
...
因此，Adam 适合作为快速建立训练基线的选择；当泛化表现是主要目标时，仍需把 SGD with momentum 纳入对照，而不能仅凭优化器名称预判结果。
...
这使衰减强度依赖参数的梯度历史，而 weight decay 原本只应由参数大小、学习率和衰减系数决定。
...
Adam 的优点是训练通常比较稳定，对学习率相对不那么敏感，也能适应不同参数梯度尺度差异较大的情况。但它需要额外维护一阶和二阶状态，也不一定在所有任务上都比 SGD 有更好的泛化表现。Adam 与 weight decay 的耦合方式还会改变正则化的含义。下一节将讨论 AdamW 如何把 weight decay 与梯度更新解耦。
