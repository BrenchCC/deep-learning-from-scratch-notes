...
title: "Chapter 4.4 RMSprop 与 Adadelta：修正学习率衰减"
order: 4
---
...
由于 $g_{\tau}^2 \ge 0$，所以 $s_{t}$ 只会增加，不会减少。
...
两者估计的对象不同：Adagrad 统计参数从训练开始以来累计了多少平方梯度，RMSprop 则估计该参数近期的梯度尺度。
...
下面实现一个简化版 RMSprop。它为每个参数维护一个 `ema_of_sq_grads` 状态，用来记录梯度平方的滑动平均。
...
需要注意的是，`zero_grad()` 清空的是参数的 `.grad` 属性，而不是 RMSprop 的 `ema_of_sq_grads`。`.grad` 是当前 mini-batch 计算得到的梯度缓存，每轮反向传播前都要清空；`ema_of_sq_grads` 是优化器的内部状态，用来记录梯度平方的滑动平均，应该跨 step 保留。这一点和 Adagrad 一样。
...
- `ema_of_sq_grads`：梯度平方的滑动平均；
- `ema_of_sq_updates`：更新量平方的滑动平均。
...
最后，我们在一个二维二次损失函数上比较 Adagrad、RMSprop 和 Adadelta 的优化轨迹。
...
在这个简单任务中，Adagrad 累积完整历史平方梯度，有效学习率持续减小，因此后期下降明显变慢。RMSprop 和 Adadelta 使用指数滑动平均，早期梯度不会一直主导后续更新。不过，两者在该任务上的轨迹差异不明显。这个实验适合解释更新机制，不能据此判断它们在复杂任务上的相对性能。

## 4.4.9 本章小结

本节从 Adagrad 的单调累计问题出发，介绍了 RMSprop 和 Adadelta。

Adagrad 为每个参数维护历史平方梯度累计量，从而实现自适应学习率。该累计量只增不减，因此有效学习率持续下降，训练后期的参数更新可能变得很小。
