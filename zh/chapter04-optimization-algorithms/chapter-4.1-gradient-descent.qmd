...
title: "Chapter 4.1 从梯度下降到 SGD"
order: 1
---

前面的章节已经说明，神经网络可以看成一个带参数的函数。给定输入 $x$，模型经过前向传播得到预测结果，再通过损失函数衡量预测和真实标签之间的差距。训练的目标，是不断调整模型参数，让损失下降。
...
这个目标看起来简单，但它背后有一个核心问题：

> **我们怎么知道参数应该往哪个方向更新？**

自动微分可以计算梯度。对于参数 $\theta$，梯度 $\nabla_\theta L(\theta)$ 描述了参数发生微小变化时损失如何变化。但是，知道梯度还不等于知道完整的训练过程。梯度只给出局部方向，真正训练模型时，还需要决定：

1. 参数应该沿着哪个方向移动；
2. 每次应该移动多远；
3. 应该用多少数据来估计这个方向。

本节先从最基础的梯度下降开始，说明“根据梯度更新参数”的基本逻辑。随后再讨论为什么深度学习中更常见的不是完整的梯度下降，而是随机梯度下降，也就是常说的 **SGD (Stochastic Gradient Descent)** [@bottou2012SGD]。
...
这里的 `x` 和 `y` 就是一个 mini-batch。`loss.backward()` 计算的是当前 mini-batch 上的梯度，并存储在模型参数的 `.grad` 属性里；`optimizer.step()` 则根据这些梯度更新参数。
...
本节还介绍了 SGD 中的 weight decay。它可以理解为在损失函数中加入 L2 正则项，让参数在降低训练损失的同时倾向于保持较小的规模。对于基础 SGD 来说，L2 regularization 和 weight decay 是等价的。但对于 Adam 这类自适应优化器，二者会产生差异，因此后面讲 AdamW 时还会重新回到这个问题。
